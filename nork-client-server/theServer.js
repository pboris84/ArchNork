...
var reader = require('readline');             // io interactions

// Server-client communication tool
var mySD = require('string_decoder').StringDecoder;  
var decodedInput = new mySD('utf8');
...
// Server initialized and connected
let host = mod.createServer((connection) => {  

    // Raw client data wrapper
    connection.on('data', (clientData) => {      
        var input = decodedInput.write(clientData).toUpperCase();  // Client data becomes string here 
        console.log("");
...
host.listen(8888, "localhost", function(err) {   
    if (err) throw err;
});
